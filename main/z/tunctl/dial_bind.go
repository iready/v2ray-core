...
// EnableDialBind 对齐 sing-box route.auto_detect_interface：TUN 活跃时绑定出站物理网卡。
// 默认网卡变化只通知 hijack 状态机，不在这里直接 Suspend/Resume。
func EnableDialBind(iface string) {
	stopDialBindWatch()
	resetHijackBits()
	setDialBindIface(strings.TrimSpace(iface))
...
func onDialBindIfaceUpdate(name string) {
	...
	if name != "" && BindInterfaceAuto() {
		setDialBindIface(name)
		syncAutoBindSnap(name)
	}
	noteDefaultIface(name)
}
