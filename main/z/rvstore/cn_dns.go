...
// DefaultSystemProbeHosts Windows NCSI 等连通性探测：不得进 FakeDNS，否则托盘变地球仪、系统判定断网。
...
// DefaultTrueResolveBypassHosts 国内 DNS 抢先真解析、且 IPv4 从 TUN 绕行的域名。
func DefaultTrueResolveBypassHosts() []string {
	out := make([]string, 0, len(DefaultNTPBypassHosts)+len(DefaultSystemProbeHosts))
	out = append(out, DefaultNTPBypassHosts...)
	out = append(out, DefaultSystemProbeHosts...)
	return out
}
