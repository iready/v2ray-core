...
func (proxy *Proxy) closeAttacker() {
	if proxy.attacker == nil {
		return
	}
	if proxy.attacker.listener != nil {
		_ = proxy.attacker.listener.Close()
	}
	if proxy.attacker.server != nil {
		_ = proxy.attacker.server.Close()
	}
	if proxy.attacker.client != nil {
		proxy.attacker.client.CloseIdleConnections()
	}
}
